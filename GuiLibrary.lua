if shared.VapeExecuted then
	...
	GuiLibrary["RemoveObject"] = function(objname)
        if GuiLibrary.ObjectsThatCanBeSaved[objname] then
	...
	local success = pcall(function()
		return cloneref(game.GetService(game, 'Players')).LocalPlayer:SetTopbarTransparency(1)
	end)
	...
	if newui then 
		local buttonbkg = Instance.new('ImageButton', GuiLibrary.MainGui)
		local rendericon = Instance.new('ImageLabel', buttonbkg)
		...
		buttonbkg.Position = UDim2.new(0.95, 0, 0.015, 0)
		...
		if inputService.KeyboardEnabled then 
			buttonbkg.ZIndex = 0.1
		end
		button = buttonbkg
		Instance.new('UICorner', buttonbkg).CornerRadius = UDim.new(1, 0)
		task.spawn(function()
			repeat task.wait() until RenderFunctions
			RenderFunctions:makedragui(button, nil, not inputService.KeyboardEnabled)
		end)
	end

	if not newui then -- else would be a bit messy yk
		local buttonmain = Instance.new('TextLabel')
		local buttonbkg = Instance.new('ImageButton', buttonmain)
		local stateoverlay = Instance.new('ImageLabel', buttonbkg) -- this is not needed but we gotta match the roblox topbar yk :troll:
		local rendericon = Instance.new('ImageLabel', buttonbkg)
		buttonmain.BackgroundTransparency = 1
		buttonmain.Size = UDim2.new(0, 44, 1, 0)
		buttonmain.Text = ''
		buttonbkg.AnchorPoint = Vector2.new(0, 1)
		buttonbkg.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
		buttonbkg.BackgroundTransparency = 1
		buttonbkg.Position = UDim2.new(0, 0, 1, 0)
		buttonbkg.Size = UDim2.new(0, 32, 0, 32)
		buttonbkg.Image = 'rbxasset://textures/ui/TopBar/iconBase.png'
		stateoverlay.BackgroundTransparency = 1
		stateoverlay.Size = UDim2.new(1, 0, 1, 0)
		stateoverlay.ZIndex = 2
		stateoverlay.Image = 'rbxasset://LuaPackages/Packages/_Index/UIBlox/UIBlox/AppImageAtlas/img_set_1x_1.png'
		stateoverlay.ImageRectOffset = Vector2.new(402, 494)
		...
		task.spawn(function()
			buttonmain.Parent = cloneref(game:FindService('CoreGui')):WaitForChild('TopBarApp'):WaitForChild('TopBarFrame'):WaitForChild('RightFrame')
		end)
		button = buttonbkg
	end

	GuiLibrary.SelfDestructEvent.Event:Once(function()
		button:Destroy()
	end)
